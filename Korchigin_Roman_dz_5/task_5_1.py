#from typing import Any, Generator
#def odd_nums(number: int) -> Generator[int, Any, None]:
#Такую конструкцию мне предложил PyCharm, чтобы исправить ошибку. Так как это меняет шапку задания, оставил
#все как есть. И данную конструкцию применил в задании 5.2.

def odd_nums(number: int) -> int:
    """Генератор, возвращающий по очереди нечетные целые числа от 1 до number (включительно)"""
    for i in range(1, number + 1, 2):
        yield i


n = 24
generator = odd_nums(n)
for _ in range(1, n + 1, 2):
    print(next(generator))
# next(generator)  # если раскомментировать, то должно падать в traceback по StopIteration